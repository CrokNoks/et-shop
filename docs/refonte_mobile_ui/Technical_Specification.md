...
| `src/components/shopping/ShoppingList.tsx`                                                                                                         | Lignes d'article 58px, anneau de coche, pas-à-pas quantité, tri par `is_purchased`, branchement mode magasin (2c/3b) sur `useAisleMode`, **et écran 4i** : le sheet d'édition d'article inline (prix unitaire dédié, unité en champ libre + raccourcis, code-barres, bouton supprimer) — corrigé après exploration, ce n'est pas `ProductForm.tsx` qui joue ce rôle (voir ligne suivante) |
...
| `src/components/shopping/ProductForm.tsx`                                                                                                          | **Correction de périmètre** : ce composant sert à créer un produit catalogue (depuis 2e/3c) ou éditer un rayon/catégorie (`StoreCategories`, `StoreCatalog`) — ce n'est **pas** l'écran 4i. Restylage léger seulement (feuille basse au lieu de panneau latéral, cf. ligne HopInput) ; les champs magasin/rayon/qté restent des `<select>`/`<input>` classiques cette itération, pas des pastilles — non traité par manque de temps, à reprendre si besoin |
...
| `src/components/purchases/PurchaseHistoryList.tsx` | idem (liste globale, utilisée uniquement par la page Historique) |
| `src/hooks/useStatistics.ts`                       | idem                                                             |

**Correction apportée en Cycle A** : `src/components/purchases/PurchaseHistoryItem.tsx`, `src/components/purchases/ProductPurchaseHistory.tsx` et `src/hooks/usePurchaseHistory.ts` sont **conservés** — ils alimentent l'historique d'achat _par produit_ affiché dans la feuille d'édition du catalogue (`StoreCatalog.tsx`), une fonctionnalité distincte de la page Historique globale et non visée par la demande de suppression. Idem pour `apps/e2e/cypress/e2e/product-purchase-history.cy.ts`, conservé.
| `src/hooks/useStatistics.ts` | idem |
