-- Sets purchased_by = auth.uid() when an item is marked purchased. Set server-side
-- inside the SECURITY DEFINER function (never accepted as a client-supplied
-- parameter) so it can't be spoofed to attribute a purchase to another user.
