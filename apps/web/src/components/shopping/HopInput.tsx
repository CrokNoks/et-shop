...
export const HopInput: React.FC<HopInputProps> = ({ listId, onItemAdded }) => {
  ...
  const handleBarcodeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.message || "Code-barres inconnu.");
    } finally { setIsAdding(false); }
  };

  const startVoiceDictation = () => {
    const SpeechRecognition = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;
    if (!SpeechRecognition) { toast.error("Navigateur non supporté."); return; }
        ...
        <SheetContent side="right" className="w-screen sm:max-w-[450px] p-10 text-[#1A365D]">
          ...
          <form onSubmit={handleBarcodeSubmit} className="space-y-8 text-[#1A365D]">
            ...
            <SheetFooter className="mt-8 pt-4 sm:justify-start">
              <Button type="submit" disabled={isAdding} className="w-full bg-[#FF6B35] hover:bg-[#e55a2b] text-white font-bold text-lg py-6 rounded-xl">{isAdding ? 'Recherche...' : 'Ajouter le produit'}</Button>
            </SheetFooter>
          </form>
        </SheetContent>
      </Sheet>
