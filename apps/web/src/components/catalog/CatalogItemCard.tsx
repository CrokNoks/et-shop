...
export const CatalogItemCard: React.FC<CatalogItemCardProps> = ({ item, onEdit, onDelete, isSelected, onSelect }) => {
  return (
    <div 
      className={`bg-white p-6 rounded-3xl border transition-all flex flex-col gap-4 group text-[#1A365D] relative ${
        isSelected ? 'border-[#FF6B35] shadow-md ring-1 ring-[#FF6B35]' : 'border-gray-50 shadow-sm hover:shadow-md'
      }`}
    >
